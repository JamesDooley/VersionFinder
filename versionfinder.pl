...
sub checkcms {
    ...
    foreach my $fingerprint ( @{ $signature->{fingerprints} } ) {
        ...
        if ( $fingerprint->{direxclude} ) {
            my $found;
            foreach my $exdir ( @{ $fingerprint->{direxclude} } ) {
                if ( $directory =~ $exdir ) {
                    _DEBUG( "Signature file match for $signame in excluded directory " . escdir($directory) );
                    $found = 1;
                    last;
                }
            }
    ...
    if ( -d "/var/cpanel" ) {
        foreach my $user ( glob("/var/cpanel/users/*") ) {
            ...
            if ( -e "/var/cpanel/suspended/$user" && !$SUSPENDED ) {
                ...
                my $dir = getUserDir($user);
                if ( $dir && -d $dir ) {
                    push( @scandirs, $dir );
                } else {
                    print "cPanel user $user did not return a valid home dir, skipping.\n";
                }
